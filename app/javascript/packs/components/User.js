...
class User extends React.Component {
  ...
  renderLists(user) {
    const { width, classes, currentSession } = this.props;
    let offset = 0;
    let limit = parseInt(process.env.LISTS_PAGE_SIZE);

    return (
      <Query query={GET_LISTS_BY_USER} variables={{ userId: user.id, offset, limit }}>
        {({ data, loading, error, fetchMore }) => {
          if (loading || error) {
            return (null);
          }

          if (data.listsByUser.length === 0) {
            return (
              <React.Fragment>
                <EmptyList
                  label={`${user.name} doesn't have any lists.`}
                />
                {
                  currentSession && currentSession.user.id === user.id &&
                    <div className={classes.newListButtonContainer}>
                      <Button
                        ...
                        onClick={() => {
                          this.setState({ newListDialog: true });
                        }}
                      >
                        Create a new user list
                      </Button>
                      <EditListDialog
                        open={this.state.newListDialog}
                        onClose={() => this.setState({ newListDialog: false })}
                        list={{ name: '', description: '', userId: user.id }}
                      />
                    </div>
                }
              </React.Fragment>
          ...
          return (
            <React.Fragment>
              <Grid container spacing={8}>
                ...
                {
                  data.listsByUser.length < user.listsCount &&
                    <LoadMoreButton
                      onClick={() => {
                        fetchMore({
                          variables: {
                            offset: data.listsByUser,
                            limit
                          },
                          updateQuery: (prev, { fetchMoreResult }) => {
                            if (!fetchMoreResult) return prev;
                            ...
                            return Object.assign({}, prev, {
                              listsByUser: [...prev.listsByUser, ...fetchMoreResult.listsByUser]
                            });
                          }
                        });
                      }}
                    ...
                    <React.Fragment>
                                ...
                                />
                                {
                                  false &&
                                    <Tab
                                      value="lists"
                                      disabled={isPrivate}
                                      label={isPrivate ? <Private /> : data.user.listsCount}
                                      icon="Lists"
                                    />
                                }
                              </Tabs>
                      ...
                      <Grid container className={classes.root} spacing={8} justify="center">
                        <Grid item item xs={12} lg={8}>
                          ...
                          {this.state.tab === 'lists' && false && this.renderLists(data.user)}
                        </Grid>
                      </Grid>
                      <EditProfileDialog
                        user={data.user}
